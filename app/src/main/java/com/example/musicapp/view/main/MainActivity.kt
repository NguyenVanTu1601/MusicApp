...
class MainActivity : AppCompatActivity(),
    ...
    private var musicBroadcast = MusicBroadcast(this)
    ...
    private fun getDataSongFromIntent() {
        val music = intent?.extras?.getParcelable<Music>(Constants.EXTRA_MUSIC) as Music
        setTextMusic(music)
    }

    private fun setTextMusic(music: Music) {
        music.let {
            nameSong = it.name.toString()
    ...
    private fun getTimeFormat(millis: Int): String {
    ...
    companion object {
        fun getIntent(context: Context, music: Music) : Intent{
            val intent = Intent(context, MainActivity::class.java)
            intent.putExtra(Constants.EXTRA_MUSIC, music)
            return intent
        }
    }
}
